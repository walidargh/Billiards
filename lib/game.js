	...
	var game = this;
		for (var i = 0; i < 100; i++) {
			...
			this.balls[i] = (new Ball(
				{
					position: [Game.DIM_X*Math.random() - 20, Game.DIM_Y*Math.random() - 20],
...
Game.prototype.inHole = function (ball) {
	...
	if ( x < 2.5*ball.radius && y < 2.5*ball.radius) {
		inHole = true;
		// Game.removeBall(ball)
	//TOPRIGHT [0 + 2.5*ball.radius, Game.DIM_X - 2.5*ball.radius] 
	} else if ( x > Game.DIM_X - 2.5*ball.radius && y < 2.5*ball.radius ) {
		inHole = true;
		// Game.removeBall(ball)
	//BOTTOMLEFT [0, DIM_Y]
	} else if ( x < 2.5*ball.radius && y > Game.DIM_Y - 2.5*ball.radius ) {
		inHole = true;
		//BOTTOM RIGHT
	} else if ( x > Game.DIM_X - 2.5*ball.radius && y > Game.DIM_Y - 2.5*ball.radius) {
		inHole = true;
	} 

	if (ball.number === 0 && inHole) {
