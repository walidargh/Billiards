...
Game.prototype.step = function (deltaTime, ctx) {
	this.checkMakes();
	this.moveAll(deltaTime*2);
	this.checkCollisions();
	if (this.allStopped()) {
		var cueBall = this.findBall(0);
		this.cue.place(ctx, cueBall);
	}
};
...
Game.prototype.allStopped = function () {
	...
	var length = Object.keys(balls).length;
...
Game.prototype.removeBall = function (ball) {
	...
	var remaining = Object.keys(this.balls);
	console.log("removed" +  ball.number + ' ' + remaining + 'remain');
};
