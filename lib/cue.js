...
Cue.prototype.place = function (cueBall) {
	...
	canvas.addEventListener("mousedown", this.beginCueListener);
	// canvas.removeEventListener("mousedown", beginCueListener);
...
Cue.prototype.beginCue = function (cueBall, event) {
	...
	var canvas = document.getElementById("canvas");
	// canvas.removeEventListener("mousedown", this.beginCueListener);
	this.velocityListener = this.calculateVelocity.bind(this, cueBall);
	canvas.addEventListener("mouseup", this.velocityListener);
};

Cue.prototype.calculateVelocity = function (cueBall, event) {
	var canvas = document.getElementById("canvas");
	canvas.removeEventListener("mouseup", this.velocityListener);
