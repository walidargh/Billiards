...
Ball.prototype.railCollision = function () {
	if (this.game.isOutOfBounds(this.position)) {
	 	var xVelocity = -1*(this.velocity[0]*BALL_BALL_RESTITUTION) //TIMES ANGLE OF COLLISION, this may result in only one velocity component being reversed in direction, depending on which wall it hits;
	 	var yVelocity = -1*(this.velocity[1]*BALL_BALL_RESTITUTION) //TIMES ANGLE OF COLLISION;
	}
};
