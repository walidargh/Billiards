...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.prototype.isOutOfBounds = function(ball) {
		if (ball.position[0] > Game.TABLE_X - ball.radius) {
			this.backInFrame(
				ball, [ball.position[0] - Game.TABLE_X + ball.radius, 0]
			);
			return [-1, 1];
		}
		...
		else if (ball.position[1] > Game.TABLE_Y - ball.radius) {
			this.backInFrame(
				ball, [0, ball.position[1] - Game.TABLE_Y + ball.radius]
			);
			return [1, -1];
		}
	...
	Game.prototype.draw = function (ctx) {
		ctx.clearRect(20, 40, Game.TABLE_X, Game.TABLE_Y);
		ctx.fillStyle = Game.BG_COLOR;
		ctx.fillRect(40, 40, Game.TABLE_X, Game.TABLE_Y);
		...
		ctx.beginPath();
		ctx.arc(
			40, Game.TABLE_Y + 40, 20, 0, 2 * Math.PI, true
		);
		ctx.fill();
	
		ctx.beginPath();
		ctx.arc(
			Game.TABLE_X + 40, 40, 20, 0, 2 * Math.PI, true
		);
		ctx.fill();
	
		ctx.beginPath();
		ctx.arc(
			Game.TABLE_X + 40, Game.DIM_Y + 40, 20, 0, 2 * Math.PI, true
		);
		ctx.fill();
