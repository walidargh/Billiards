...
/***/ function(module, exports, __webpack_require__) {
	...
	var Weapon = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"./weapon\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
	...
	Game.prototype.inHole = function (ball) {
		...
		if ( x < 4*ball.radius && y < 4*ball.radius) {
			inHole = true;
			// Game.removeBall(ball)
		//TOPRIGHT [0 + 2.5*ball.radius, Game.DIM_X - 2.5*ball.radius] 
		} else if ( x > Game.DIM_X - 4*ball.radius && y < 4*ball.radius ) {
			inHole = true;
			// Game.removeBall(ball)
		//BOTTOMLEFT [0, DIM_Y]
		} else if ( x < 4*ball.radius && y > Game.DIM_Y - 4*ball.radius ) {
			inHole = true;
			//BOTTOM RIGHT
		} else if ( x > Game.DIM_X - 4*ball.radius && y > Game.DIM_Y - 4*ball.radius) {
			inHole = true;
		} 
		return inHole;
	};
