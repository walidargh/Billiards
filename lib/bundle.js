...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.prototype.draw = function (ctx, event) {
		ctx.clearRect(0, 0, Game.DIM_X + Game.offSet, Game.DIM_Y + Game.offSet);
		ctx.fillStyle = "#FFFFFF";
		ctx.fillRect((Game.offSet)/2 - 40, (Game.offSet)/2 - 40, Game.DIM_X + 80, Game.DIM_Y + 80);
		...
		ctx.beginPath();
		ctx.arc(
			Game.DIM_X + Game.offSet/2, Game.DIM_Y + Game.offSet/2, 28, 0, 2 * Math.PI, true
		);
		ctx.fill();
		ctx.closePath();
	...
	Cue.prototype.draw = function (ctx, event) {
		if (this.cueDrawnBack) {
			...
			if (event && !this.cueMoveForward) {
				...
				this.cueDistance = 0.25*Util.distance(this.cueBall.position, cueEnd);
			}
			// console.log(-cueAngle);
			console.log(this.cueDistance);
			ctx.save();
...
/***/ function(module, exports, __webpack_require__) {
	...
	GameView.prototype.animate = function (time, event) {
		...
		if (this.moved) this.game.draw(this.ctx, this.event, this.cueImg);
		else this.game.draw(this.ctx, this.cueImg);
		this.moved = false;
