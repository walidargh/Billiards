...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.positions =
	{
		1: [Game.DIM_X/3 + 4*13, Game.DIM_Y/2],
	
		2: [Game.DIM_X/3 + 2*13, Game.DIM_Y/2 - 1*13],
		3: [Game.DIM_X/3 + 2*13, Game.DIM_Y/2 + 1*13],
	
		4: [Game.DIM_X/3 + 0*13, Game.DIM_Y/2 - 2*13],
		5: [Game.DIM_X/3 + 0*13, Game.DIM_Y/2 - 0*13],
		6: [Game.DIM_X/3 + 0*13, Game.DIM_Y/2 + 2*13],
	
		7: [Game.DIM_X/3 - 2*13, Game.DIM_Y/2 - 3*13],
		8: [Game.DIM_X/3 - 2*13, Game.DIM_Y/2 - 1*13],
		9: [Game.DIM_X/3 - 2*13, Game.DIM_Y/2 + 1*13],
		10: [Game.DIM_X/3 - 2*13, Game.DIM_Y/2 + 3*13],
	
		11: [Game.DIM_X/3 - 4*13, Game.DIM_Y/2 - 4*13],
		12: [Game.DIM_X/3 - 4*13, Game.DIM_Y/2 - 2*13],
		13: [Game.DIM_X/3 - 4*13, Game.DIM_Y/2 + 0*13],
		14: [Game.DIM_X/3 - 4*13, Game.DIM_Y/2 + 2*13],
		15: [Game.DIM_X/3 - 4*13, Game.DIM_Y/2 + 4*13]
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	Ball.prototype.isCollidedWith = function (otherBall) {
		...
		if (distanceBetween <= (this.radius + otherBall.radius)) { 
			...
			console.log('This Ball: ' + 
									...
									scaleFactorA + ' ' + 
									speedA + ' ' + sumOfSpeeds + ' ' + 
									moveBackDistanceA + ' ' + moveBackDistanceB + ' ');
			console.log('-------------------------------')
			console.log('OtherBall: ' + otherBall.position + ' ' + backUpVectorB + ' ' + scaleFactorB + ' ' + otherBall.velocity + ' ' + scaleFactorB);
	...
	Ball.prototype.move = function (deltaTime=50) {
	...
	Ball.prototype.tableFriction = function (deltaTime=50) {	
		...
		// var frictionScale = -1*GRAVITY*BALL_TABLE_FRICTION*deltaTime;
		var ballSpeed = Util.norm(this.velocity);
		var frictionOffsetSpeed = 
			1/(BALL_TABLE_FRICTION_MAX - BALL_TABLE_FRICTION_MIN);
		var kineticFactor = 
			(1 / (Math.pow(ballSpeed, 2) + frictionOffsetSpeed) + BALL_TABLE_FRICTION_MIN);
