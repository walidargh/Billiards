...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.DIM_X = 1000;
	Game.DIM_Y = 500;
	Game.TABLE_X = Game.DIM_X + 40;
	Game.TABLE_Y = Game.DIM_Y + 40;
	Game.RAILS = {
		left: 20,
		right: Game.DIM_X + 20,
		top: 20, 
		bottom: Game.DIM_Y + 20
	};
	// Game.OffSet = 40;
	Game.FPS = 32;
	...
	Game.prototype.draw = function (ctx) {
		ctx.clearRect(20, 20, Game.DIM_X, Game.DIM_Y);
		ctx.fillStyle = Game.BG_COLOR;
		ctx.fillRect(20, 20, Game.DIM_X, Game.DIM_Y);
	
		ctx.fillStyle = "#000000";
		
		ctx.beginPath();
		ctx.arc(
			35, 35, 25, 0, 2 * Math.PI, true
		);
		ctx.fill();
	
		ctx.beginPath();
		ctx.arc(
			35, Game.DIM_Y + 5, 25, 0, 2 * Math.PI, true
		);
		ctx.fill();
	
		ctx.beginPath();
		ctx.arc(
			Game.DIM_X + 5, 35, 25, 0, 2 * Math.PI, true
		);
		ctx.fill();
	
		ctx.beginPath();
		ctx.arc(
			Game.DIM_X + 5, Game.DIM_Y + 5, 25, 0, 2 * Math.PI, true
		);
		ctx.fill();
	...
	Game.prototype.inHole = function (ball) {
		...
		if (Util.distance(ball.position, topLeft) < 2*ball.radius) {
			inHole = true;
			// Game.removeBall(ball)
		//TOPRIGHT [0 + 2.5*ball.radius, Game.DIM_X - 2.5*ball.radius] 
		} else if (Util.distance(ball.position, topRight) < 2*ball.radius) {
			inHole = true;
			// Game.removeBall(ball)
		//BOTTOMLEFT [0, DIM_Y]
		} else if (Util.distance(ball.position, bottomLeft) < 2*ball.radius) {
			inHole = true;
			//BOTTOM RIGHT
		} else if (Util.distance(ball.position, bottomRight) < 2*ball.radius) {
			inHole = true;
		} 
		return inHole;
	};
